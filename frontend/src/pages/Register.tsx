...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Campos comunes por rol
      if (rol === "cliente") {
        payload.telefono = telefono;     // aunque no sea required, lo mandamos
        payload.direccion = direccion;   // texto libre
      }

      if (rol === "vendedor") {
        payload.telefono = telefono;     // <- IMPORTANTE, si no la validación peta
        payload.zona = zona;
      ...
      if (rol === "vendedor") {
        //  Vendedor: no auto-login; solo aviso y a login
        await alertSuccess(
          "Solicitud enviada",
        ...
        //  Cliente: auto-login y refresca navbar
        localStorage.setItem("token", data.token);
        localStorage.setItem("user", JSON.stringify(data.user));
        window.dispatchEvent(new Event("auth:changed")); //  importante para Navbar
